...
use crate::components::{AddDeviceModal, SavedSection, SendDataModal, TopBar};
use crate::engine::bridge::{self, EngineSender, TransferState};
...
pub fn App() -> Element {
    ...
    // Channel for receiving transfer-state updates from the engine
    // event listener (which runs on a tokio::spawn task and therefore
    // cannot touch Dioxus signals directly).
    let mut event_rx: Signal<Option<mpsc::UnboundedReceiver<TransferState>>> = use_signal(|| None);
    ...
    use_effect(move || {
        ...
        match bridge::start_engine(&app_config.listen_addr) {
            Ok(handle) => {
                // Spawn the event listener — it pushes TransferState
    ...
    // ── Pump engine events into the Dioxus signal on the main thread ──
    // use_future runs on the Dioxus async runtime (same thread as
    // signals) so we can safely call .set() here.
    use_future(move || async move {
        loop {
            // Try to receive a state update without holding the write
